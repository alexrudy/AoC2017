...
impl MemoryPosition {
  fn shift(&self, dx : int, dy : int) -> MemoryPosition {
    MemoryPosition { x:self.x + dx, y:self.y + dy }
  }
...
fn neighbor_sum(position: &MemoryPosition, grid: &HashMap<MemoryPosition, uint>) -> Result<uint,String> {
  let mut nsum : uint = 0;
  for dx in -1..2 {
    for dy in -1..2 {
      nsum = nsum.checked_add(*grid.get(&position.shift(dx, dy)).unwrap_or(&0)).ok_or("Overflow Error")?;
    }
  }
  Ok(nsum)
}
...
pub fn first_large_value(position: uint) -> Result<uint, String> {
  ...
  for _i in 1.. {
    ...
    grid.insert(pos, val);
    if !grid.contains_key(&MemoryPosition { x: pos.x - dd.1, y: pos.y + dd.0}) {
      dd = (-1 * dd.1, dd.0)
    }
    if val > position {
      return Ok(val);
