...
impl Generator {
  ...
  pub fn new(factor: uint, start: uint) -> Generator {
    Generator { factor: factor, prevvalue: start, picky: None}
  }
...
mod tests {
  ...
  fn example_part1_first_few() {
    ...
    assert_eq!(a.take(5).collect::<Vec<uint>>(), vec![1092455, 1181022009, 245556042, 1744312007, 1352636452]);
    assert_eq!(b.take(5).collect::<Vec<uint>>(), vec![430625591, 1233683848, 1431495498, 137874439, 285222916]);
  }
  ...
  fn example_part2_first_few() {
    ...
    a.picky(4);
    b.picky(8);
    assert_eq!(a.take(5).collect::<Vec<uint>>(), vec![1352636452, 1992081072, 530830436, 1980017072, 740335192]);
    assert_eq!(b.take(5).collect::<Vec<uint>>(), vec![1233683848, 862516352, 1159784568, 1616057672, 412269392]);
  }
