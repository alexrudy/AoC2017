...
pub fn run_instructions(instructions: &mut Vec<isize>, position: isize, jumper: fn(&mut Vec<isize>, isize) -> Result<isize, String>) -> Result<isize, String> {
  let mut nsteps = 0;
...
mod tests {
  ...
  fn jump_maze() {
    ...
    assert_eq!(run_instructions(&mut instructions, 0, jump_instruction).unwrap(), 5);
  }
  
  #[test]
  fn jump_fancy_maze() {
    let mut instructions = vec![0, 3, 0, 1, -3];
    assert_eq!(run_instructions(&mut instructions, 0, jump_fancy).unwrap(), 10);
  }
