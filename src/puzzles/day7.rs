...
impl graph::Graph<Program> {
  ...
  pub fn parse_node(&mut self, text: &str) -> Result<(graph::Node, Vec<String>), graph::GraphError> {
    let mut parts = text.trim().split(char::is_whitespace);
    let name = parts.next().ok_or(graph::GraphError::NodeParseError(text.to_string()))?.to_string();
    
    
    let weight = {
      let weight_text = parts.next().ok_or(graph::GraphError::NodeParseError(text.to_string()))?;
  ...
  pub fn parse_nodes<Q: io::BufRead>(&mut self, lines: io::Lines<Q>) -> Result<(), graph::GraphError> {
    ...
    for (node, children) in children_map {
      let child_nodes : Vec<graph::Node> = children.iter().map(|x| node_map.get(&x.to_string()).unwrap().clone()).collect();
...
impl graph::Node {
  ...
  pub fn fixed_weight(&self, nodes: &graph::Graph<Program>) -> Option<u32> {
    self.target_weight(nodes).map(|x| {x - self.children(nodes).iter().map(|x| {x.weight(nodes)}).sum::<u32>()})
  }
}
...
mod tests {
  ...
  fn parse_program_test() {
    ...
    let children_expected : Vec<String> = vec!["ktlj", "cntj", "xhth"].iter().map(|s| s.to_string()).collect();
    ...
    let node_expected = Program { name : "fwft".to_string(), weight: 72};
