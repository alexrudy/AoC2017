...
fn part_one() {
  ...
  assert_eq!(
    day24::bridges(&connectors).map(|b| b.strength()).max(),
    Some(1906)
  );
}
...
fn part_two() {
  ...
  let connectors = get_connectors().expect("Error loading connectors");
  let (_bl, bs) = day24::bridges(&connectors)
...
fn get_connectors() -> Result<Vec<day24::Connector>, Box<Error>> {
  Ok(puzzles::get_puzzle_reader(24)?
    .lines()
    .map(|l| Ok(l?.parse::<day24::Connector>()?))
    .collect::<Result<Vec<day24::Connector>, Box<Error>>>()?)
}
